	...
	public void paintComponent(Graphics g)
	{
		...
		if(this.produitsPositions != null && this.produitsImages != null)
		{
			...
			for(int i = 0; i< this.produitsPositions.size();i++)
			{
				...
				g.drawImage(this.produitsImages.get(i), pointProduit.x, pointProduit.y, null);
				pointProduit.translate(this.produitsVitesses.get(i).x, this.produitsVitesses.get(i).y);
				g.fillOval(pointProduit.x, pointProduit.y, taille, taille);
			}
		}
		position.translate(vitesse.x, vitesse.y);
		g.fillRect(position.x, position.y, taille, taille);
